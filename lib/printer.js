...
async function rasterPrint(canvas) {
  await BLE.send(CMD_ENABLE);      await sleep(300);
  await BLE.send(CMD_WAKE);        await sleep(300);
  await BLE.send(CMD_DENSITY);     await sleep(200);
  await BLE.send(CMD_LABEL_START); await sleep(200);
  await BLE.send(canvasToBitmapCmd(canvas)); await sleep(2000);
  await BLE.send(CMD_LABEL_POS);   await sleep(300);
  await BLE.send(CMD_LABEL_END);   await sleep(200);
  await BLE.send(CMD_FEED);        await sleep(3000);
...
export async function testPrint() {
  ...
  ctx.fillText('=== TEST PRINT ===', W / 2, 10);
  ctx.font = '24px Arial';
  ctx.fillText('BLE raster OK', W / 2, 52);
  ctx.fillText('UniverCell', W / 2, 86);
  await rasterPrint(c);
}
