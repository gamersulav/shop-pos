...
const SERVICES = [
  ...
  '0000ffe0-0000-1000-8000-00805f9b34fb',  // HM-10 BLE UART module (very common)
  '0000ae30-0000-1000-8000-00805f9b34fb',  // Deli / similar brands
  '000018f0-0000-1000-8000-00805f9b34fb',  // SPP-like service
  '0000fee7-0000-1000-8000-00805f9b34fb',  // Some label printers
...
export async function connect() {
  ...
  // Enumerate every service the device advertises (for discovery / error reporting)
  let allServices = [];
  try { allServices = await server.getPrimaryServices(); } catch {}

  // Try known services first, then any discovered extras
  const knownSet  = new Set(SERVICES);
  const extraUuids = allServices.map(s => s.uuid).filter(u => !knownSet.has(u));

  for (const uuid of [...SERVICES, ...extraUuids]) {
    try {
  ...
  if (!_char) {
    const found = allServices.map(s => s.uuid);
    const hint  = found.length
      ? `\n\nServices found on this device:\n${found.join('\n')}\n\nSend this to support so the correct UUID can be added.`
      : '\n\nCould not enumerate device services.';
    throw new Error(`Printer connected but no writable characteristic found.${hint}`);
  }
