...
// ── Protocol probe ────────────────────────────────────────────────────────────
// Tries ESC/POS text then ESC/POS raster (both work over HM-10 ffe1 UART).
// Watch the printer — tell us which letter(s) A / B come out.
export async function testPrint() {
  ...
  BLE.resetQueue();
  await BLE.send(new Uint8Array([0x1b, 0x40]));   // ESC @ init
  await sleep(100);
  await BLE.send(new Uint8Array([             // "HELLO A\n"
    0x48,0x45,0x4c,0x4c,0x4f,0x20,0x41,0x0a,
  ]));
  await BLE.send(FEED);
  await sleep(3000);
  ...
  await sleep(100);
  const bc = document.createElement('canvas');
  bc.width = W; bc.height = 64;
  const bctx = bc.getContext('2d');
  bctx.fillStyle = '#fff'; bctx.fillRect(0, 0, W, 64);
  bctx.fillStyle = '#000'; bctx.textBaseline = 'top'; bctx.textAlign = 'center';
  bctx.font = 'bold 48px Arial'; bctx.fillText('B', W / 2, 8);
  await BLE.send(canvasToBitmapCmd(bc));
  await sleep(500);
  await BLE.send(FEED);
  await sleep(3000);
}
