...
// ── Protocol probe — tries 3 approaches so we find the right one in one shot ──
// Approach A: plain ESC/POS text, no wrapper
// Approach B: ESC/POS raster (GS v 0 bitmap), no wrapper
// Approach C: AiYin enable (10 FF FE 01) + raster + stop (10 FF FE 45)
// Watch the printer — whichever letter(s) A/B/C come out tells us the protocol.
export async function testPrint() {
  if (!BLE.isConnected()) throw new Error('Printer not connected.');
  const W = PRINT_WIDTH;

  function tinyCanvas(letter) {
    ...
    ctx.fillText(letter, W / 2, 8);
    return c;
  }

  const FEED = new Uint8Array([0x1b, 0x4a, 0x50]);

  // Approach A — plain ESC/POS text
  BLE.resetQueue();
  await BLE.send(new Uint8Array([0x1b, 0x40]));          // ESC @
  await sleep(100);
  await BLE.send(new Uint8Array([0x41, 0x0a]));           // 'A' + LF
  await BLE.send(FEED);
  await sleep(3000);

  // Approach B — raw ESC/POS raster (GS v 0), no wrapper
  BLE.resetQueue();
  await BLE.send(new Uint8Array([0x1b, 0x40]));
  await sleep(100);
  await BLE.send(canvasToBitmapCmd(tinyCanvas('B')));
  await sleep(500);
  await BLE.send(FEED);
  await sleep(3000);
  ...
  await sleep(200);
  await BLE.send(canvasToBitmapCmd(tinyCanvas('C')));
  await sleep(500);
