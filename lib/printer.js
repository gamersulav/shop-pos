...
// ── Print canvas: TSPL framing + raw bitmap bytes after CLS ──────────────────
// All explicit drawing commands (TEXT/BARCODE/BITMAP/BAR) are ignored by this
// firmware. Hypothesis: after CLS the printer reads bpr*PH raw raster bytes
// then expects PRINT — same as many budget Chinese label printers.
async function printCanvas(canvas) {
  ...
  const bitmap = new Uint8Array(bpr * PH);
  for (let r = 0; r < PH; r++) {
    for (let c = 0; c < PW; c++) {
      const i = (r * PW + c) * 4;
      const lum = img.data[i]*0.299 + img.data[i+1]*0.587 + img.data[i+2]*0.114;
      if (lum < 128) bitmap[r * bpr + (c >> 3)] |= 0x80 >> (c & 7);
    }
  }

  const enc = new TextEncoder();
  const header = enc.encode(`SIZE 50 mm,30 mm\r\nGAP 3 mm,0 mm\r\nCLS\r\n`);
...
export async function printProductLabel(product) {
  // SELFTEST prints the printer's own config label — shows firmware & protocol
  const enc = new TextEncoder();
  await send(enc.encode('SELFTEST\r\n'));
