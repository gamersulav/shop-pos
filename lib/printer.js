...
// ── Send canvas bitmap to printer via TSPL BITMAP command ────────────────────
async function printCanvas(canvas) {
  const ctx = canvas.getContext('2d');
  const img = ctx.getImageData(0, 0, PW, PH);
  const bpr = PW >> 3; // 48 bytes per row (384 dots / 8)
  ...
  // TSPL: SIZE/GAP/CLS, then BITMAP command (binary data immediately after the comma)
  ...
  const header = enc.encode(
    ...
    'DIRECTION 0\r\n' +
    'CLS\r\n' +
    'BITMAP 0,0,48,240,0,'   // width=48 bytes, height=240 dots, mode=0 (overwrite)
  );
