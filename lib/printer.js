...
// ── Niimbot packet builder ────────────────────────────────────────────────────
function niimPkt(cmd, data) {
  const body = [cmd, (data.length >> 8) & 0xff, data.length & 0xff, ...data];
  const xor  = body.reduce((a, b) => a ^ b, 0);
  return new Uint8Array([0x55, 0x55, ...body, xor, 0xAA, 0xAA]);
}

// ── Print canvas: TSPL framing + Niimbot 0x85 row packets ────────────────────
// TSPL drawing commands are all silently ignored by this firmware.
// The ff00 service is the Niimbot characteristic — setup via TSPL, image
// data via Niimbot row packets (cmd 0x85), then TSPL PRINT to fire.
async function printCanvas(canvas) {
  const ctx = canvas.getContext('2d');
  const img = ctx.getImageData(0, 0, PW, PH);
  const bpr = PW >> 3; // 50 bytes per row

  // 1-bit bitmap: bit=1 → black dot, MSB = leftmost pixel
  const bitmapRows = [];
  for (let r = 0; r < PH; r++) {
  ...
  // TSPL label setup (works on this printer)
  await send(enc.encode(`SIZE 50 mm,30 mm\r\nGAP 3 mm,0 mm\r\nCLS\r\n`));
  await ms(100);

  // Niimbot start-print signal
  await send(niimPkt(0x01, [0x00, 0x00]));
  await ms(50);

  // Niimbot row data packets (cmd 0x85: [row_hi, row_lo, ...50_bytes])
  for (let r = 0; r < PH; r++) {
    const row = bitmapRows[r];
    const data = new Uint8Array(2 + bpr);
    data[0] = (r >> 8) & 0xff;
    data[1] = r & 0xff;
    data.set(row, 2);
    await send(niimPkt(0x85, data));
    if ((r & 15) === 15) await ms(3);
  }
  await ms(200);

  // Niimbot end-print, then TSPL PRINT to eject
  await send(niimPkt(0xF3, [0x00]));
  await ms(50);
  await send(enc.encode(`PRINT 1,1\r\n`));
  await ms(500);
}
...
export async function printProductLabel(product) {
  ...
  // DIAGNOSTIC: solid black rectangle — no fonts, tests bitmap+protocol only
  ctx.fillStyle = '#000';
  ctx.fillRect(0, 0, PW, PH);       // entire label black
  ctx.fillStyle = '#fff';
  ctx.fillRect(20, 20, PW - 40, PH - 40); // white inset so border is visible
