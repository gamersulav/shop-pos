...
// ── Send canvas bitmap via TSPL BITMAP (data after command CRLF) ──────────────
// Binary data comes after the command's own \r\n so the parser reads it by
// byte-count (width×height) instead of scanning for a CRLF terminator.
async function printCanvas(canvas) {
  const ctx = canvas.getContext('2d');
  const img = ctx.getImageData(0, 0, PW, PH);
  const bpr = PW >> 3; // 48 bytes per row
  ...
  // BITMAP command ends with \r\n; binary data follows immediately (no comma)
