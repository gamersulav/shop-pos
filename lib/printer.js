...
async function printCanvas(canvas) {
  ...
  // 1-bit bitmap: S441 uses 0=black dot, 1=white — invert so dark canvas pixels print
  const bitmapRows = [];
  for (let r = 0; r < PH; r++) {
    const row = new Uint8Array(bpr).fill(0xFF); // start all-white (1s)
    for (let c = 0; c < PW; c++) {
      const i = (r * PW + c) * 4;
      const lum = img.data[i]*0.299 + img.data[i+1]*0.587 + img.data[i+2]*0.114;
      if (lum < 128) row[c >> 3] &= ~(0x80 >> (c & 7)); // clear bit → print dot
    }
    bitmapRows.push(row);
  }
