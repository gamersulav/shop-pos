...
async function printCanvas(canvas) {
  ...
  // Packet = 0x55 0x55 + cmd(1) + len(2) + data(2+bpr) + xor(1) + 0xAA 0xAA = 10+bpr bytes
  const rowBuf = new Uint8Array(PH * (10 + bpr));
  let off = 0;
  for (let r = 0; r < PH; r++) {
    const pkt = niimPkt(0x85, [(r >> 8) & 0xff, r & 0xff, ...bitmapRows[r]]);
    rowBuf.set(pkt, off);
    off += pkt.length;
  }
  await send(rowBuf.slice(0, off));
  await ms(300);
