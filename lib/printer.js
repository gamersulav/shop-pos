...
// ── Canvas (48×30 mm at 203 DPI = 384×240 dots) ───────────────────────────────
const PW = 384;
const PH = 240;
...
async function printCanvas(canvas) {
  ...
  // Hex-encode (pure ASCII — avoids all binary/CRLF parser issues)
  const hex = Array.from(bitmap, b => b.toString(16).padStart(2, '0').toUpperCase()).join('');
  const total = bpr * PH; // 11520

  // ZPL: print-width, label-length, graphic field at origin
  const zpl = `^XA^PW${PW}^LL${PH}^FO0,0^GFA,${total},${total},${bpr},${hex}^XZ\n`;

  await send(new TextEncoder().encode(zpl));
  await ms(600);
}
