...
// ── Send canvas bitmap to printer via ESC/POS raster ─────────────────────────
async function printCanvas(canvas) {
  const ctx = canvas.getContext('2d');
  const img = ctx.getImageData(0, 0, PW, PH);
  const bpr = PW >> 3; // 384 / 8 = 48 bytes per row
  ...
  await send(new Uint8Array([0x1B, 0x40]));
  await ms(120);

  // GS v 0 — raster bit image (normal density, 48 bytes/row, 240 rows)
  const hdr = new Uint8Array([
    0x1D, 0x76, 0x30, 0x00,
    bpr & 0xFF, (bpr >> 8) & 0xFF,
    PH  & 0xFF, (PH  >> 8) & 0xFF,
  ]);
  const payload = new Uint8Array(hdr.length + bitmap.length);
  payload.set(hdr);
  payload.set(bitmap, hdr.length);
  await send(payload);
  await ms(400);

  // FF — form feed / eject label
  await send(new Uint8Array([0x0C]));
}
