...
function call(method, ...args) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      delete window._btCallbacks[id];
      reject(new Error(`${method}: no response (is printer on and connected?)`));
    }, 8000);
...
// Test print: ESC @ init + text + cut — isolates write path from label/receipt logic
export async function printTest() {
  const ESC = 0x1B, GS = 0x1D;
  await send(merge([
    new Uint8Array([ESC, 0x40]),                                   // init/wake
    new TextEncoder().encode('Bluetooth Test OK\r\n\r\n\r\n\r\n'), // CR+LF
    new Uint8Array([GS, 0x56, 0x42, 0x00]),                       // cut/feed
  ]));
}
...
// ── Repair ticket (50×30 mm, Font B compact) ─────────────────────────────────
// Font B = ESC M 1 → 9×17 dots per char → ~42 chars/line on 50 mm paper
// 30 mm height ≈ 14 lines at Font B → keep content minimal

function buildRepairTicket({ customer_name, customer_phone, phone_model, issue, customer_price, repair_discount, payment_method, shopName }) {
  const shop  = shopName || 'UNIVERCELL';
  const final = Math.max(0, Number(customer_price || 0) - Number(repair_discount || 0));
  const div   = '--------------------------------\n';
  // truncate long strings to fit 42-char Font-B line
  const trunc = (s, n) => s && s.length > n ? s.slice(0, n - 1) + '…' : (s || '');

  return merge([
    b(ESC, 0x40),                          // init
    b(ESC, 0x4D, 0x01),                    // Font B (small/compact)
    b(ESC, 0x61, 0x01),                    // center
    b(ESC, 0x45, 0x01), t(shop + '\n'), b(ESC, 0x45, 0x00),
    t('Mobile Acc & Repair\n'),
    b(ESC, 0x61, 0x00),                    // left
    t(div),
    t(`Customer: ${trunc(customer_name, 30)}\n`),
    customer_phone ? t(`Phone: ${customer_phone}\n`) : new Uint8Array(0),
    t(`Device: ${trunc(phone_model, 30)}\n`),
    t(`Issue: ${trunc(issue, 34)}\n`),
    t(div),
    b(ESC, 0x45, 0x01),
    t(`Total: Rs ${final.toLocaleString()}\n`),
    b(ESC, 0x45, 0x00),
    t(`Payment: ${payment_method || 'Cash'}\n`),
    t(div),
    b(ESC, 0x61, 0x01), t('Thank you!\n'),
    b(ESC, 0x4D, 0x00),                    // back to Font A
    t('\n'),                               // minimal feed before cut (keeps height ~30 mm)
    b(GS, 0x56, 0x42, 0x00),              // cut
  ]);
}

export async function printRepairTicket(repair) { await send(buildRepairTicket(repair)); }
