...
// ── Repair ticket (compact, 50×30 mm) ────────────────────────────────────────
// Uses only Font A + the same ESC/POS commands as buildReceipt (proven to work).
// Content kept to 8 lines so ticket stays within ~30 mm at 203 DPI.
...
function buildRepairTicket({ customer_name, customer_phone, phone_model, issue, customer_price, repair_discount, payment_method, shopName }) {
  ...
  // combine name + phone on one line to save space
  const namePhone = customer_phone
    ? trunc(`${customer_name} | ${customer_phone}`, 32)
    : trunc(customer_name, 32);

  return merge([
    b(ESC, 0x40),                                              // init/reset
    b(ESC, 0x61, 0x01), b(ESC, 0x45, 0x01),
    t(shop + '\n'),
    b(ESC, 0x45, 0x00), b(ESC, 0x61, 0x00),
    t(div),
    t(namePhone + '\n'),
    t(trunc(phone_model, 32) + '\n'),
    t(trunc(issue, 32) + '\n'),
    t(div),
    b(ESC, 0x61, 0x02), b(ESC, 0x45, 0x01),
    t(`TOTAL: Rs ${final.toLocaleString()}\n`),
    b(ESC, 0x45, 0x00),
    b(ESC, 0x61, 0x02), t(`Payment: ${payment_method || 'Cash'}\n`),
    b(ESC, 0x61, 0x01), t('\nThank you!\n\n\n'),              // same feed as buildReceipt
